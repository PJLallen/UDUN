...
def _prepare_data(pred: np.ndarray, gt: np.ndarray) -> tuple:
    """
    A numpy-based function for preparing ``pred`` and ``gt1``.

    - for ``pred``, it looks like ``mapminmax(im2double(...))`` of matlab;
    - ``gt1`` will be binarized by 128.
...
class Smeasure(object):
    ...
    def region(self, pred: np.ndarray, gt: np.ndarray) -> float:
        ...
        # assert np.isclose(w1 + w2 + w3 + w4, 1), (w1 + w2 + w3 + w4, pred.mean(), gt1.mean())
    ...
    def divide_with_xy(self, pred: np.ndarray, gt: np.ndarray, x: int, y: int) -> dict:
        """
        Use (x,y) to divide the ``pred`` and the ``gt1`` into four submatrices, respectively.
        """
        h, w = gt.shape
        area = h * w
...
class Emeasure(object):
    ...
    def cal_em_with_threshold(self, pred: np.ndarray, gt: np.ndarray, threshold: float) -> float:
        ...
        ``[pred attribute(foreground fg, background bg)]_[gt1 attribute(foreground fg, background bg)]_[meaning]``

        If only ``pred`` or ``gt1`` is considered, another corresponding attribute location is replaced with '``_``'.
        """
...
class WeightedFmeasure(object):
    ...
    def cal_wfm(self, pred: np.ndarray, gt: np.ndarray) -> float:
        ...
        # 注意这里使用mask索引矩阵的时候不可使用Ew[gt1]，这实际上仅在索引Ew的0维度
        R = 1 - np.mean(Ew[gt == 1])
        P = TPw / (TPw + FPw + _EPS)
